...
export const getHistory = () => {
  const url = "http://34.205.76.141:8000/history/getall";
  return Axios.get(url, {
    headers: { "x-access-token": "bearer " + localStorage.getItem("token") },
  });
};
...
export const getMoreMenu = (page) => {
  const url = `http://34.205.76.141:8000/?page=${page}&limit=6`;
  return Axios.get(url);
};
export const deleteMenu = (id) => {
  const url = `http://34.205.76.141:8000/?id=${id}`;
...
export const editMenu = (name, picture, price, id_category, id_menu) => {
  ...
  const url = "http://34.205.76.141:8000/";
